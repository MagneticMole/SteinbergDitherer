...
color_thief = ColorThief("./in/" + file_path)
...
for bands in output:
    with open("./CLR/"+ sys.argv[2] + "-" + split[0] + ".txt", "w+") as dump:
        dump.write(str(bands[0]) + " " + str(bands[1]) + " " + str(bands[2]) + "\n")
...
class Steinberg:
    ...
    def generatePalette(self):
        self.palette = np.genfromtxt("./CLR/"+ sys.argv[2] + "-" + split[0] + ".txt", usecols=(2, 1, 0), dtype=int)
